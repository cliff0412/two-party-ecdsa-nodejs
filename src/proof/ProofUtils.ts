...
export class ProofUtils {
     ...
     */
    public static generatePaillierPublicKeyProof(N: BN, p: BN, q: BN): PaillierPublicKeyProof {
        ...
        let phi: BN = p.sub(CryptoConsants.ONE).mul(q.sub(CryptoConsants.ONE));
        let NInv: BN;
        try {

            let nInRed = N.toRed(BN.red(phi));
            NInv = nInRed.redInvm();
        } catch (e) {
            throw new Error(CryptoException.INCONSISTENT_INPUTS + e.toString());
        }
    ...
    */
    public static verifyPaillierPublicKeyProof(proof: PaillierPublicKeyProof): boolean {
        if (proof == null) {
            throw new Error(CryptoException.NULL_INPUT);
        }

        let sigma = proof.getSigma();
        let N = proof.getN();
        ...
        for (let s of sigma) {
            ...
            s = (s as any).fromRed()
            let sInRed = s.toRed(BN.red(N))
            let sPowN = sInRed.redPow(N)
    ...
    public static verifyECDlogProof(
        ...
        let n: BN = this.ec.n as BN;
        let e: BN = this.computeChallenge(n, G, proof.getQ(), proof.getX());
    ...
    */
    ...
    private static generateBigInteger(m: BN, in_val: BN): BN {
        ...
        let out = new ArrayBuffer(num);
        let out_view = new Uint8Array(out);

        let temp = in_val.toBuffer();
        let count = 0;
        while (num >= 32) {   // 32 is the digest length in bytes
            md.update(temp)
            temp = md.digest();
