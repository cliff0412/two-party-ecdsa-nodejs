...
export class SigningP2 {
    ...
    public constructor(
        ...
        this.p1EcdsaPrivateKeyShareEncryption = context.getP1EcdsaPrivateKeyShareEncryption();
    }
    ...
    public computePublicRandom(privateRandomShare: BN, p1PublicRandomShare: ECPoint): ECPoint {
    ...
    public computeC3(z: BN, r: BN, privateRandomShare: BN): BN {
        ...
        let rho = random.randBetween(CryptoConsants.ONE, n.mul(n).sub(CryptoConsants.ONE));

        let k2Inv = privateRandomShare.toRed(BN.red(n)).redInvm();
        let c1 = Paillier.encrypt(this.p1PaillierPublicKey, rho.mul(n).add(k2Inv.mul(z).mod(n)));

        let v = k2Inv.mul(r).mul(this.ecdsaPrivateKeyShare).mod(n);
        let c2 = Paillier.multiply(this.p1PaillierPublicKey, this.p1EcdsaPrivateKeyShareEncryption, v);
