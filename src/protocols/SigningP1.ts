...
export class SigningP1 {
  ...
  public computeSignature(
    ...
    const n = CryptoConsants.SECP256_CURVE_N;
    const R = p2PublicRandomShare.mul(privateRandomShare);

    //BigInteger r = R.normalize().getXCoord().toBigInteger().mod(n);
    const r = R.getX().mod(n);
    ...
    const recoveryParam =
      (R.getY().isOdd() ? 1 : 0) | (R.getX().cmp(r) !== 0 ? 2 : 0);

    if (
      ellipticUtil.verifySig(
        z,
        r,
        s,
