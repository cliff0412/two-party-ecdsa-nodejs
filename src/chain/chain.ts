...
let providerUrl = "https://kovan.infura.io/v3/b666e55354c042c589008dd116351623"
...
export const recover = (msgHash: Buffer, sig: Signature): string => {
    return web3.eth.accounts.recover({
        ...
        v: '0x' + new BN(ellipticUtil.formSigVval(sig.recovery as number, CryptoConsants.CHAIN_NETWORK_ID_KOVAN), 10).toString("hex"),
    ...
    return new Promise((resolve, reject) => {
        ...
        web3.eth.getTransactionCount(from)
            .then(nonce => {
                ...
                var tx = new Transaction(rawTx, { 'chain': 'kovan' });


                let hash = tx.hash(false)
     
                let sig: Signature = sign(signingContextP1, signingContextP2, new BN(hash))


                // TEMP ************************************************************ */
                let listOfVs = [0, 1]
                let recoverred = false;
                for (let vTemp of listOfVs) {
                    sig.recovery = vTemp;
                ...
                let sigOnChain: SignatureOnChain = {
                    r: sig.r,
                    s: sig.s,
                    v: ellipticUtil.formSigVval(sig.recovery as number, CryptoConsants.CHAIN_NETWORK_ID_KOVAN)
                }
