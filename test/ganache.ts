...
export default class Ganache {
    public static port = 8545;
    private server: ganache.Server | undefined = undefined;
    private static accounts = [
        ...
        secretKey,
        balance: ethers.utils.parseUnits('10000').toHexString(),
    }));
    public static ganache: Ganache;

    constructor(port = Ganache.port) {
        Ganache.port = port;
        Ganache.ganache = this;
    }
    ...
    static get signers() {
        ...
        return this.accounts.map(
            ({ secretKey }) => new ethers.Wallet(secretKey, provider),
        );
    }
    ...
    async start(): Promise<void> {
        ...
        this.server = ganache.server({ accounts: Ganache.accounts });

        return new Promise((resolve, reject) => {
            if (this.server) {
                this.server.listen(Ganache.port, (err?: Error) => {
                    if (err) {
                        reject(err);
                    } else {
                        resolve();
