import BN from 'bn.js';
import {
  KeyGenContextVO, PaillierPrivateKey, PaillierPublicKey, random, SigningContextP1,
  SigningContextP2
} from '../src';
import { CryptoConsants } from '../src/common/CryptoConstants';
import { keyGen, sign } from './fixture/two-party-ecdsa';

describe('two party ecdsa', () => {

  it('keygen', async () => {
    const keyGenCtx = await keyGen();

    expect(keyGenCtx.addressFromPoint).toBeDefined()
  });

  it('signing', async () => {
    const keyGenCtx: KeyGenContextVO = await keyGen();

    // let keyGenCtx: KeyGenContextVO = {
    //     "addressFromPoint": "0x7e3D67bab611E25b4Fc9D31C0aEe20cc852e40DF",
    //     "addressFromX": "0x7e3D67bab611E25b4Fc9D31C0aEe20cc852e40DF",
    //     "x": "af82ad81d61763e45dda07091ebc903a2c8cecf9fe2f8404df60abf2c2ffc8ae",
    //     "p1": {
    //         "x1": "deb881f5fd4c35a231ad6b438d43e62f7031d9b80d6ead95f297e517241ca5e",
    //         "paillierPrivateKeyP": "",
    //         "paillierPrivateKeyQ": ""
    //     },
    //     "p2": {
    //         "paillierPublicKeyN": "",
    //         "cKey": "",
    //         "x2": "18492c2f59f3e088279caddeedbe07898d24101eabfeb50bb841089e201fb422"
    //     }
    // }
    const x1 = new BN(keyGenCtx.p1.x1, 'hex');
    const x2 = new BN(keyGenCtx.p2.x2, 'hex');
    const x = x1.mul(x2).mod(CryptoConsants.SECP256_CURVE_N);
    const Q = CryptoConsants.SECP256_CURVE_G.mul(x);
    const signingContextP1 = new SigningContextP1(
      new PaillierPrivateKey(
        new BN(keyGenCtx.p1.paillierPrivateKeyP, 'hex'),
        new BN(keyGenCtx.p1.paillierPrivateKeyQ, 'hex'),
      ),
      x1,
      Q,
    );

    const signingContextP2 = new SigningContextP2(
      new PaillierPublicKey(new BN(keyGenCtx.p2.paillierPublicKeyN, 'hex')),
      new BN(keyGenCtx.p2.cKey, 'hex'),
      x2,
      Q,
    );
    // hash of message to be signed
    const z = random.randBetween(
      CryptoConsants.ONE,
      CryptoConsants.SECP256_CURVE_N.sub(CryptoConsants.ONE),
    );

    const sig = sign(signingContextP1, signingContextP2, z);
    expect(sig.r.gt(CryptoConsants.ZERO));
  });
})


