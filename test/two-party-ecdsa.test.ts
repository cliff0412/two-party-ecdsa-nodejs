...
const keyGen = async (saveKeyGenCtx: boolean = false) => {
    ...
    if (saveKeyGenCtx) {
        let keyGenTtx: KeyGenContext = {
            ...
            x: keyGenContextP1.getEcdsaPrivateKeyShare().mul(keyGenContextP2.getEcdsaPrivateKeyShare()).mod(CryptoConsants.SECP256_CURVE_N),
            Q: p1Q
        }
...
export const sign = (
    ...
    let sig: Signature = p1Sign.computeSignature(z, signingContextP1.getP2EcdsaPublicRandomShare(), c3, signingContextP1.getEcdsaPrivateRandomShare());
...
test('signing', async () => {
    ...
    let sig = sign(signingContextP1, signingContextP2, z)
